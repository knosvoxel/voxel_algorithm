...
bool rayBoxIntersection(const Ray& ray, const Grid3D& grid, double& tMin, double& tMax, double t0, double t1)
{
    double tYMin, tYMax, tZMin, tZMax;
    const double x_inv_dir = 1 / ray.direction().x;
    if(x_inv_dir >= 0){
    ...
    const double y_inv_dir = 1 / ray.direction().y;
    if(y_inv_dir >= 0){
    ...
    const double z_inv_dir = 1 / ray.direction().z;
    if(z_inv_dir >= 0){
...
void voxelAlgorithm(const Ray& ray, const Grid3D& grid, double t0, double t1){
    double tMin, tMax;
    ...
    tMin = MAX(tMin, t0);
    tMax = MAX(tMax, t1);
    const glm::vec3 ray_start = ray.origin() + ray.direction() * tMin;
    const glm::vec3 ray_end = ray.origin() + ray.direction() * tMax;
    ...
    else if(ray.direction().x < 0.0)
    {
        ...
        tMaxX = tMin + (grid.minBound().x + previous_X_index * grid.voxelSizeX() - ray_start.x) / ray.direction().x;
    }
    else
    {
        stepX = 0;
    ...
    if (ray.direction().y > 0.0)
    {
        ...
        tMaxY = tMin + (grid.minBound().y + current_Y_index * grid.voxelSizeY() - ray_start.y) / ray.direction().y;
    }
    else if(ray.direction().y < 0.0)
    {
        ...
        tMaxY = tMin + (grid.minBound().y + previous_Y_index * grid.voxelSizeY() - ray_start.y) / ray.direction().y;
    }
    else
    {
        stepY = 0;
    ...
    {
        stepZ = 1;
        tDeltaY = grid.voxelSizeZ() / ray.direction().z;
        tMaxZ = tMin + (grid.minBound().z + current_Z_index * grid.voxelSizeZ() - ray_start.z) / ray.direction().z;
    }
    else if(ray.direction().z < 0.0)
    {
        ...
        tMaxZ = tMin + (grid.minBound().z + previous_Z_index * grid.voxelSizeZ() - ray_start.z) / ray.direction().z;
    }
    else
    {
        stepZ = 0;
